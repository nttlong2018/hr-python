...
def get_list_with_searchtext(args):
    ...
    if(searchText != None):
        ret.match("contains(tr_supplier_name, @name)",name=searchText)
    ...
    try:
        ...
        if args['data'] != None:
            ...
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = TrainSupplier.display_list_train_supplier().match("_id == {0}", ObjectId(args['data']['_id'])).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  models.HCSLS_TrainSupplier().delete("_id in {0}",[ObjectId(x["_id"])for x in args['data']])
            lock.release()
            return ret
