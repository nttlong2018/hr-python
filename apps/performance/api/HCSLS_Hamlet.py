...
def get_list_with_searchtext(args):
    ...
    if(searchText != None):
        ret.match("contains(hamlet_name, @name)",name=searchText)
    ...
    try:
        ...
        if args['data'] != None:
            ...
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = AdministrativeSubdivisions.get_hamlet().match("_id == {0}", ObjectId(args['data']['_id'])).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  models.HCSLS_Hamlet().delete("_id in {0}",[ObjectId(x["_id"])for x in args['data']])
            lock.release()
            return ret
