(function (scope) {
    ...
    scope.tableFields = [
        { "data": "quit_job_code", "title": "${get_res('quit_job_code_table_header','Mã')}" },
        { "data": "quit_job_name", "title": "${get_res('quit_job_name_table_header','Tên')}" },
    ...
    function onEdit() {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Detail_QuitJob','Chi tiết Hình thức thôi việc')}", 'category/form/addQuitJob', function () { });
        } else {
    ...
    function onDelete() {
        if (!scope.selectedItems || scope.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_QuitJob/delete')}")
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_QuitJob/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
    ...
    function _comboboxData() {
        services.api("${get_api_key('app_main.api.SYS_ValueList/get_list')}")
            ...
            .then(function (res) {
                ...
                scope.cbbQuitJobType = res.values;
                scope.$applyAsync();
            })
    }
    _comboboxData();
