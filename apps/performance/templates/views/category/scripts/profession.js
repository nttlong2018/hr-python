(function (scope) {
    ...
    function onEdit() {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Detail_Profession','Chi tiết Nghề nghiệp chuyên môn')}", 'category/form/addProfession', function () { });
        } else {
    ...
    function onDelete() {
        if (!scope.selectedItems || scope.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_Profession/delete')}")
    ...
    function onExport() {
        lv.ExportFile("/excel_export")
            .data({
                'collection_name': 'HCSLS_Profession'
            }).done();
    }
    function onImport() {
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_Profession/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
