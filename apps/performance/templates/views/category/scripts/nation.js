(function (scope) {
    ...
    scope.tableFields = [
        { "data": "nation_code", "title": "${get_res('nation_code_table_header','Mã')}" },
        { "data": "nation_name", "title": "${get_res('nation_name_table_header','Tên')}" },
    ...
    function onEdit() {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Detail_Nation','Chi tiết quốc gia')}", 'category/form/addNation', function () { });
        } else {
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_Nation/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
