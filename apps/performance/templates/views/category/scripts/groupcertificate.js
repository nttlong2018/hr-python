(function (scope) {
    ...
    scope.tableFields = [
        { "data": "group_cer_code", "title": "${get_res('group_cer_code_table_header','Mã')}" },
        { "data": "group_cer_name", "title": "${get_res('group_cer_name_table_header','Tên')}" },
    ...
    function onEdit() {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Detail_GroupCertificate','Chi tiết Nhóm chứng chỉ')}", 'category/form/addGroupCertificate', function () { });
        } else {
    ...
    function onDelete() {
        if (!scope.selectedItems || scope.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_GroupCertificate/delete')}")
    ...
    function onExport() {
        lv.ExportFile("/excel_export")
            .data({
                'collection_name': 'HCSLS_GroupCertificate'
            }).done();
    }
    function onImport() {
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_GroupCertificate/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
    ...
    function _comboboxData() {
        services.api("${get_api_key('app_main.api.SYS_ValueList/get_list')}")
            .data({
                //parameter at here
                "name": "LGroupCertificate"
            })
            .done()
            .then(function (res) {
                delete res.language;
                delete res.list_name;
                scope.cbbGroupCerType = res.values;
                scope.$applyAsync();
            })
    }
    _comboboxData();
