(function (scope) {
    ...
    scope.tableFields = [
        { "data": "train_level_code", "title": "${get_res('train_level_code_table_header','Mã')}" },
        { "data": "train_level_name", "title": "${get_res('train_level_code_table_header','Tên')}" },
    ...
    function onEdit() {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Detail_Acadame','Trình độ học vấn')}", 'category/form/addAcadame', function () {
                setTimeout(function () {
    ...
    function onDelete() {
        if (!scope.selectedItems || scope.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_Acadame/delete')}")
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_Acadame/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
    ...
    function _comboboxData() {
        services.api("${get_api_key('app_main.api.SYS_ValueList/get_list')}")
            ...
            .then(function (res) {
                ...
                scope.LAcadameRange = res.values;
                services.api("${get_api_key('app_main.api.SYS_ValueList/get_list')}")
                    .data({
                        //parameter at here
                        "name": "LAcadameCoEff"
                    ...
                    .then(function (res) {
                        delete res.language;
                        delete res.list_name;
                        scope.LAcadameCoEff = res.values;
                        services.api("${get_api_key('app_main.api.SYS_ValueList/get_list')}")
                            .data({
                                //parameter at here
                                "name": "LBeginDateCal"
                            ...
                            .then(function (res) {
                                delete res.language;
                                delete res.list_name;
                                scope.LBeginDateCal = res.values;
                                scope.$applyAsync();
                            })
                    })
            })
