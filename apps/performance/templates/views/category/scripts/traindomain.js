(function (scope) {
    ...
    scope.tableFields = [
        { "data": "domain_code", "title": "${get_res('domain_code_table_header','Mã')}" },
        { "data": "domain_name", "title": "${get_res('domain_name_table_header','Tên')}" },
    ...
    function onEdit() {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Detail_TrainDomain','Chi tiết Lĩnh vực đào tạo')}", 'category/form/addTrainDomain', function () { });
        } else {
    ...
    function onDelete() {
        if (!scope.selectedItems || scope.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_TrainDomain/delete')}")
    ...
    function onExport() {
        lv.ExportFile("/excel_export")
            .data({
                'collection_name': 'HCSLS_TrainDomain'
            }).done();
    }
    function onImport() {
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_TrainDomain/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
