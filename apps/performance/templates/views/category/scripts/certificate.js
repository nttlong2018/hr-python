(function (scope) {
    ...
    scope.tableFields = [
        { "data": "cer_code", "title": "${get_res('cer_code_table_header','Mã')}" },
        { "data": "cer_name", "title": "${get_res('cer_name_table_header','Tên')}" },
    ...
    function onEdit() {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Detail_Certificate','Chi tiết Chứng chỉ')}", 'category/form/addCertificate', function () { });
        } else {
    ...
    function onDelete() {
        if (!scope.selectedItems || scope.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_Certificate/delete')}")
    ...
    function onExport() {
        lv.ExportFile("/excel_export")
            .data({
                'collection_name': 'HCSLS_Certificate'
            }).done();
    }
    function onImport() {
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_Certificate/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
