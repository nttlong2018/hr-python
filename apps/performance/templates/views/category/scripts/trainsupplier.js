(function (scope) {
    ...
    scope.tableFields = [
        { "data": "tr_supplier_code", "title": "${get_res('tr_supplier_code_table_header','Mã')}" },
        { "data": "tr_supplier_name", "title": "${get_res('tr_supplier_name_table_header','Tên')}" },
    ...
    function onEdit() {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Detail_TrainSupplier','Chi tiết Nơi đào tạo')}", 'category/form/addTrainSupplier', function () { });
        } else {
    ...
    function onDelete() {
        if (!scope.selectedItems || scope.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_TrainSupplier/delete')}")
    ...
    function onExport() {
        lv.ExportFile("/excel_export")
            .data({
                'collection_name': 'HCSLS_TrainSupplier'
            }).done();
    }
    function onImport() {
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_TrainSupplier/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
